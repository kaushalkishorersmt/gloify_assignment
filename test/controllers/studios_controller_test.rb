...
class StudiosControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create studio" do
    assert_difference('Studio.count') do
      post studios_url, params: { studio: { status: @studio.status, title: @studio.title, user_id: @studio.user_id } }
    end
  ...
  test "should update studio" do
    patch studio_url(@studio), params: { studio: { status: @studio.status, title: @studio.title, user_id: @studio.user_id } }
